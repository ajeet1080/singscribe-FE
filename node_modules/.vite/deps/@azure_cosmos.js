...
function shouldEnable(logger7) {
  return Boolean(azureLogLevel && levelMap[logger7.level] <= levelMap[azureLogLevel]);
}
...
var IndexUtilizationInfo = class _IndexUtilizationInfo {
  ...
  static createFromString(delimitedString, isBase64Encoded) {
    ...
    return (_a3 = result.result) !== null && _a3 !== void 0 ? _a3 : _IndexUtilizationInfo.Empty;
  }
};
...
var DiagnosticNodeInternal = class _DiagnosticNodeInternal {
  ...
  recordQueryResult(resources, level) {
    var _a3;
    if (allowTracing(level, this.diagnosticLevel)) {
      const previousCount = (_a3 = this.data.queryRecordsRead) !== null && _a3 !== void 0 ? _a3 : 0;
...
function calculateResponsePayloadLength(response) {
  var _a3;
  return ((_a3 = response === null || response === void 0 ? void 0 : response.bodyAsText) === null || _a3 === void 0 ? void 0 : _a3.length) || 0;
}
...
var QueryIterator = class {
  ...
  needsQueryPlan(error) {
    var _a3;
    if (((_a3 = error.body) === null || _a3 === void 0 ? void 0 : _a3.additionalErrorInfo) || error.message.includes("Cross partition query only supports")) {
...
var logger4 = createClientLogger("core-rest-pipeline");
...
function generateUUID() {
  ...
  for (let i = 0; i < 32; i++) {
    ...
    if (i === 12) {
      ...
      uuid4 += randomNumber & 3 | 8;
    } else {
      uuid4 += randomNumber.toString(16);
    }
...
// node_modules/@azure/core-rest-pipeline/dist-esm/src/util/typeGuards.js
...
// node_modules/@azure/core-rest-pipeline/dist-esm/src/util/file.js
var rawContent = Symbol("rawContent");

// node_modules/@azure/core-rest-pipeline/dist-esm/src/policies/multipartPolicy.js
var validBoundaryCharacters = new Set(`abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'()+,-./:=?`);

// node_modules/@azure/core-rest-pipeline/node_modules/@azure/abort-controller/dist-esm/src/AbortError.js
...
function delay2(delayInMs, value, options) {
  return new Promise((resolve, reject) => {
    ...
    const rejectOnAbort = () => {
      return reject(new AbortError2((options === null || options === void 0 ? void 0 : options.abortErrorMsg) ? options === null || options === void 0 ? void 0 : options.abortErrorMsg : StandardAbortMessage));
    };
...
async function makeRequest(request2) {
  ...
  try {
    ...
    if (isBlob2(request2.body) && request2.onUploadProgress) {
...
async function buildPipelineResponse(httpResponse, request2, abortControllerCleanup) {
  ...
  if (
    ...
    ((_a3 = request2.streamResponseStatusCodes) === null || _a3 === void 0 ? void 0 : _a3.has(Number.POSITIVE_INFINITY)) || ((_b2 = request2.streamResponseStatusCodes) === null || _b2 === void 0 ? void 0 : _b2.has(response.status))
  ) {
...
function setupAbortSignal(request2) {
  ...
  if (request2.abortSignal) {
    ...
    abortControllerCleanup = () => {
      var _a3;
      if (abortListener) {
        (_a3 = request2.abortSignal) === null || _a3 === void 0 ? void 0 : _a3.removeEventListener("abort", abortListener);
      }
    };
  }
...
function buildRequestBody(request2) {
  const body = typeof request2.body === "function" ? request2.body() : request2.body;
  if (isNodeReadableStream(body)) {
    throw new Error("Node streams are not supported in browser environment.");
  }
  return isWebReadableStream(body) ? { streaming: true, body: buildBodyStream(body, { onProgress: request2.onUploadProgress }) } : { streaming: false, body };
}
...
function bearerTokenAuthenticationPolicy(options) {
  ...
  const logger7 = options.logger || logger4;
  ...
  return addDignosticChild(async (localDiagnosticNode) => {
    ...
    try {
      ...
      if (err.code === StatusCodes.ENOTFOUND || err.code === "REQUEST_SEND_ERROR" || err.code === StatusCodes.Forbidden && (err.substatus === SubStatusCodes.DatabaseAccountNotFound || err.substatus === SubStatusCodes.WriteForbidden)) {
        retryPolicy2 = retryPolicies.endpointDiscoveryRetryPolicy;
      } else if (err.code === StatusCodes.TooManyRequests) {
        retryPolicy2 = retryPolicies.resourceThrottleRetryPolicy;
      } else if (err.code === StatusCodes.NotFound && err.substatus === SubStatusCodes.ReadSessionNotAvailable) {
        retryPolicy2 = retryPolicies.sessionReadRetryPolicy;
      } else if (err.code === StatusCodes.ServiceUnavailable || err.code === TimeoutErrorCode) {
        retryPolicy2 = retryPolicies.timeoutFailoverRetryPolicy;
      } else {
        retryPolicy2 = retryPolicies.defaultRetryPolicy;
      }
      const results = await retryPolicy2.shouldRetry(err, localDiagnosticNode, retryContext, requestContext.endpoint);
      if (!results) {
        ...
        await sleep(retryPolicy2.retryAfterInMs);
        return execute({
          diagnosticNode,
...
// node_modules/@azure/cosmos/dist-esm/src/request/RequestHandler.js
var logger5 = createClientLogger("RequestHandler");
...
async function httpRequest(requestContext, diagnosticNode) {
  ...
  if (response.status >= 400) {
    ...
    logger5.warning(response.status + " " + requestContext.endpoint + " " + requestContext.path + " " + result.message);
...
var logger6 = createClientLogger("ClientContext");
...
var ClientContext = class {
  ...
  async queryFeed({ path, resourceType, resourceId, resultFn, query, options, diagnosticNode, partitionKeyRangeId, partitionKey, startEpk, endEpk }) {
    ...
    this.applySessionToken(request2);
    logger6.info("query " + requestId + " started" + (request2.partitionKeyRangeId ? " pkrid: " + request2.partitionKeyRangeId : ""));
    logger6.verbose(request2);
    const start = Date.now();
    const response = await RequestHandler.request(request2, diagnosticNode);
    logger6.info("query " + requestId + " finished - " + (Date.now() - start) + "ms");
...
var CosmosClient = class {
  constructor(optionsOrConnectionString) {
    ...
    if (((_a3 = optionsOrConnectionString.connectionPolicy) === null || _a3 === void 0 ? void 0 : _a3.enableEndpointDiscovery) && ((_b2 = optionsOrConnectionString.connectionPolicy) === null || _b2 === void 0 ? void 0 : _b2.enableBackgroundEndpointRefreshing)) {
